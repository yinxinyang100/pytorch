...
class TestCuda(TestCase):
    ...
    def test_cublas_use_fp16_accumulation_get_set(self):
        orig = torch.backends.cuda.matmul.use_fp16_accumulation
        self.assertEqual(
            torch._C._get_cublas_use_fp16_accumulation(), orig
        )
        torch.backends.cuda.matmul.use_fp16_accumulation = not orig
        self.assertEqual(
            torch._C._get_cublas_use_fp16_accumulation(), not orig
        )
        torch.backends.cuda.matmul.use_fp16_accumulation = orig
