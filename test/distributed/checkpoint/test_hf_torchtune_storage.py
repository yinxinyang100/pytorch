...
from torch.distributed.checkpoint.hf_torchtune_storage import (
    HuggingFaceHubTorchTuneReader,
    HuggingFaceHubTorchTuneWriter,
    SUFFIX,
)
...
class TestHfTorchTuneStorage(TestCase):

    def test_write_data_hf(self) -> None:
        with tempfile.TemporaryDirectory() as path:
            writer = HuggingFaceHubTorchTuneWriter(
                path=path,
            )
            ...
            expected_write_results = [
                WriteResult(index=MetadataIndex(fqn='tensor_0', offset=None, index = None), size_in_bytes=ANY, storage_data="model-00001-of-00001"),
             WriteResult(index=MetadataIndex(fqn='tensor_1', offset=None, index = None), size_in_bytes=ANY, storage_data="model-00001-of-00001")]
    ...
    def test_read_data_hf(self) -> None:
        with tempfile.TemporaryDirectory() as path:
            reader = HuggingFaceHubTorchTuneReader(path=path)
            setattr(reader, 'fs', FileSystem())
            name = "tensor_0"
            file_name = "model-00001-of-00001"
            tensor_0 = torch.rand(4)
            save_file({name: tensor_0}, os.path.join(path, file_name + SUFFIX))
